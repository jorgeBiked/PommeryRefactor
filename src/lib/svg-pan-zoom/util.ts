...
export function maxOffset(offset: number, windowDimension: number, imageDimension: number) {
...
export function getBoundedTouchTransform(
  ...
  const extendPercentage = 0.1
  const extendLimit = (viewDim.width * extendPercentage)
  const extendLimitH = (viewDim.height * extendPercentage)
  ...
  if (scaledCanvas.width < viewDim.width &&
    ...
    maxBounds = {
      ...
      y: (viewDim.height - scaledCanvas.height) + extendLimitH - zoomDisplacement.y
    }

    minBounds = {
      x: - zoomDisplacement.x - extendLimit,
      y: - zoomDisplacement.y - extendLimitH
    }
    ...
    minBounds = {
      x: viewDim.width - zoomDisplacement.x - (scaledCanvas.width + extendLimit),
      y: Dimensions.get('window').height - (scaledCanvas.height + zoomDisplacement.y + extendLimitH)
    }
  }
